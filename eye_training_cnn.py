...
if __name__ == '__main__':
    pickle_files = ['trained_model/open_eyes.pickle', 'trained_model/closed_eyes.pickle'] #File is selected and opened
    i = 0
    for pickle_file in pickle_files: #For loop reads all the dataset one by one
    ...
    print('Training set', train_dataset.shape, train_labels.shape) #If all images which were there are read or not
    print('Test set', test_dataset.shape, test_labels.shape)

    batch_size = 30 #30 images will be read once 
    nb_classes = 1 #
    nb_epoch = 12 #Iteration - 12 times the 1D array will be read
    ...
    model.add(Conv2D(32, (3, 3), padding='same',
                            input_shape=(img_channels, img_rows, img_cols))) # images are read here
    ...
    model.add(Conv2D(64, (3, 3), padding='same')) #14
    model.add(Activation('relu'))
    ...
    model.add(Conv2D(128, (3, 3), padding='same')) #1
    model.add(Activation('relu'))
    model.add(Conv2D(128, (3, 3)))
    model.add(Activation('relu'))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Dropout(0.25))
    # Here we get 1D array
